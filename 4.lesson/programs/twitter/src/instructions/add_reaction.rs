...
pub fn add_reaction(ctx: Context<AddReactionContext>, reaction: ReactionType) -> Result<()> {
    ...
    match reaction {
        ReactionType::Like => {
            tweet.likes = match tweet.likes.checked_add(1) {
                Some(likes) => likes,
                None => return Err(TwitterError::MaxLikesReached.into()),
            };
        }
        ReactionType::Dislike => {
            tweet.dislikes = match tweet.dislikes.checked_add(1) {
                Some(dislikes) => dislikes,
                None => return Err(TwitterError::MaxDislikesReached.into()),
            };
        }
    };
